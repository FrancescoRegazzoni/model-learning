function VisualizeANN(numn,w,theta,alpha,beta,norms)
%clear
%load network.mat; normalizza = 0;
%load network2.mat; normalizza = 1;
%load network8.mat; normalizza = 1;
%N = numnF(end);
%%
% numn = numnG;
% w = wG;
...
xGap = .5;
text(Xpoints(1)-xGap,Ypoints(1),...
        'Ca $\rightarrow$','Interpreter','latex','HorizontalAlignment', 'right');
text(Xpoints(2)-xGap,Ypoints(2),...
        'SL $\rightarrow$','Interpreter','latex','HorizontalAlignment', 'right');
for i = 1:numn(1)-2
    iPoint = 2+i;
    text(Xpoints(iPoint)-xGap,Ypoints(iPoint),...
        sprintf('${x}_%d \\rightarrow$',i),'Interpreter','latex','HorizontalAlignment', 'right')
end
for i = 1:numn(end)
    iPoint = sum(numn(1:end-1))+i;
    text(Xpoints(iPoint)+xGap,Ypoints(iPoint),...
        sprintf('$\\rightarrow \\dot{x}_%d$',i),'Interpreter','latex')
end
