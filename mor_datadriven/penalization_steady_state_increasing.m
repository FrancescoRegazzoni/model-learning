function pen_handler = penalization_steady_state_increasing(optionsfile)

    Pen_s           = iniread(optionsfile,'Penalizations','pen_ss_incr','s','0');
    
    pen_handler.num_pen = 1;
    pen_handler.names = {'ERRssIncr'};
    ...
    function pen_handler = initialize(pen_handler,problem,N_,N_alpha_,useG_,nS,nwF_,nwG,nAL,nIC,misc)
        if pen_handler.pen_active
            ...
            if useG_
                error('penalization_steady_state_increasing wotks only for useG_ = false')
            end
...
%             d_penal_func = @(x) 2/ep*(x<=ep).*(x/ep-1);
            ...
            for i_point = 1:nsamples
                ...
                [Du,Dx,Duw,Dxw] = modelclass.eval_sensitivity_steady_state_incr(paramsF,x_curr,u_curr,a_curr);
                
                res(i_point) = penal_func(-Du/Dx);
                if compJac || compGrad
                    jac(i_point,:) = d_penal_func(-Du/Dx) * (Du*Dxw - Dx*Duw) / Dx^2;
                end
            end
